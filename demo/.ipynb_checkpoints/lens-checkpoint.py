...
def show_future_lens(model, tok, prefix, context, in_layer = 13, tgt_in_layer = 13, topk = 5, num_toks_gen=4, context_pos = 9, color=None, remote=True):
    print("came to show future lens", flush=True)
    ...
    with model.trace(max_new_tokens=num_toks_gen, pad_token_id=50256, remote=remote) as tracer:
        with tracer.invoke(prefix) as invoker:
            transplant_hs = model.transformer.h[in_layer].output.t[prefix_pos].save()
            overall_hs = []
            init_logits = []
            for i in range(num_layers):
                print("came to layer", i, flush=True)
                init_logits.append(get_scores(model, model.transformer.h[i].output).save())
                overall_hs.append(model.transformer.h[i].output.save())
    
    output = tracer.output
    print(tok.decode(output[0][prefix_pos+1:]), flush=True)
    ...
    for curr_hs in hs:
        ...
        for x in range(0, curr_hs.shape[1]):
            ...
            with model.generate(max_new_tokens=num_toks_gen, pad_token_id=50256, remote=remote) as generator2:
                future_output_logits = list().save() # Initialize & .save() nnsight list
                # Call .all() to apply intervention to each new token
                with tracer.all():
                    # Apply intervention - set first layer output to zero
                    model.transformer.h[tgt_in_layer].output.t[context_pos] = sub_hs
                    # Append desired hidden state post-intervention
                    future_output_logits.append(model.lm_head.output) # no need to call .save
            ...
            curr_future_outputs.append([tok.decode(curr_fav_tok)])
            curr_future_preds.append([curr_fav_tok_pred[0].item()])
